...
def generate(text, count, check):
    ...
    if len(images) > 0:
        ...
        for l in range(len(img_transform)):
            ...
            if check:
                new_crop_img = Image.new("RGB", (img.width+20, img.height+20), "white")
                rand_dis_1 = random.randint(0,20)
                rand_dis_2 = random.randint(0,20)
...
# '茨城県高萩市']
# labels_0 = ['愛知県一宮市', '千葉県市川市', '兵庫県神戸市灘区' , '山形県寒河江市', '愛知県名古屋市中川区', '愛知県犬山市']
# labels_1 = ['東京都大田区', '東京都品川区', '東京都国立市', '東京都西東京市', '東京都豊島区']
# labels_2 = ['大阪府堺市東区', '大阪府泉大津市', '大阪府大阪市東住吉区', '大阪府東大阪市', '大阪府河内長野市']
# labels_3 = ['北海道上川郡鷹栖町', '北海道旭川市', '北海道札幌市東区', '北海道滝川市', '北海道江別市']
...
for count, label in enumerate(labels):
    for la in tqdm(label):
        for num in range(160):
            ...
            # print(num)
            path = '/home/anlab/Tienanh-backup/TrainHWJapanese/data/28062022_text_detection/dataset_TH7/dataset_7_addition_label_2007/'
            # if os.path.isdir(path)==False: os.mkdir(path)
            mask_1, x1, y1, x2, y2, size_width_1, size_height_1  = generate(la, num, True)
            ...
            # print(x1, y1, x2, y2, size_width_1, size_height_1)
            x_hat,y_hat,w_hat,h_hat = pascal_voc_to_yolo(x1,y1, x2, y2,size_width_1,size_height_1)
            with open('/home/anlab/Tienanh-backup/TrainHWJapanese/data/28062022_text_detection/dataset_TH7/label_dataset_7_addition_label_2007/'+la+str(num) + '.txt', 'w', encoding='utf-8') as f:
